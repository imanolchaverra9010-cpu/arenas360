...
def validate_database_url(raw_url: str | None = None) -> None:
    """Reject Railway private URLs — external hosts like Render need the public proxy URL."""
    url = normalize_database_url(raw_url or os.getenv("DATABASE_URL", ""))
    hostname = (urlparse(url).hostname or "").lower()

    if hostname.endswith(".railway.internal") or hostname == "postgres.railway.internal":
        raise RuntimeError(
            "DATABASE_URL uses Railway's private host (postgres.railway.internal). "
            "Render cannot reach it. In Railway → PostgreSQL → Connect, copy the "
            "PUBLIC URL (host like *.proxy.rlwy.net), not the private/internal one."
        )
